...
void SLAudioPlay::PlayCall(void *bufq) {
    ...
    mux.lock();
    if (pcmQue && (*pcmQue)) (*pcmQue)->Enqueue(pcmQue, buf, d.size);
    mux.unlock();
    d.Drop();
}
...
void SLAudioPlay::Close() {
    ...
    mux.lock();
    //停止播放
    if (iplayer && (*iplayer)) (*iplayer)->SetPlayState(iplayer, SL_PLAYSTATE_STOPPED);
    // 清理播放队列
    if (pcmQue && (*pcmQue)) (*pcmQue)->Clear(pcmQue);
    // 销毁player对象
    if (player && (*player)) (*player)->Destroy(player);
    // 销毁混音器
    if (mix && (*mix)) (*mix)->Destroy(mix);
    //销毁播放引擎
    if (engineSL && (*engineSL)) (*engineSL)->Destroy(engineSL);
